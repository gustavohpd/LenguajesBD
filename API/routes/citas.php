...
function createCita() {
    ...
    $estado_id   = isset($input['estado_id']) ? (int)$input['estado_id'] : 1;
    $fecha_hora  = $input['fecha_hora']; // string "YYYY-MM-DD HH:mm:ss"
...
function deleteCita($id) {
    ...
    if (oci_execute($stid)) {
        oci_commit($conn);
        echo json_encode(['message' => 'Cita eliminada (estado=2)']);
    } else {
        $e = oci_error($stid);
        http_response_code(500);
        echo json_encode([
